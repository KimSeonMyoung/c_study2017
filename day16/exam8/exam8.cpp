...
REAL g_fAngle, g_fHeadAngle2, g_bullet;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case VK_LEFT:
			/*if (g_fAngle == -90) {
				g_vPosition -= (irr::core::vector2df(1, 0) * 10);
			}
			else {
				g_fAngle = -90;
			}*/
    ...
    case WM_PAINT:
        {
			...
			//탱크 그리기
			{
				...
				// 포탑
				grp.RotateTransform(g_fHeadAngle2);	
				grp.DrawRectangle(&pen, -5, -40, 10, 45);
				grp.DrawRectangle(&pen, -10, -10, 20, 30);
