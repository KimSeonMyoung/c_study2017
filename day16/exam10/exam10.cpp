    ...
    case WM_PAINT:
        {
			...
			plusEngine::printf(&grp, line1.start.X, line1.start.Y, L"(%.lf , %.lf)", line1.start.X, line1.start.Y);
			plusEngine::printf(&grp, line1.end.X, line1.end.Y, L"(%.lf , %.lf)", line1.end.X, line1.end.Y);
			...
			if (line1.intersectWith(line2, colPt)) {
				plusEngine::printf(&grp, colPt.X, colPt.Y, L"(%.lf , %.lf)", colPt.X, colPt.Y);
				grp.DrawRectangle(&pen, colPt.X - 4, colPt.Y - 4, 8., 8.);
			}
			else {
				/*line1.getClosestPoint(colPt);
				plusEngine::printf(&grp, colPt.X, colPt.Y, L"(%.lf , %.lf)", colPt.X, colPt.Y);
				grp.DrawRectangle(&pen, colPt.X - 4, colPt.Y - 4, 8., 8.);*/
			}
