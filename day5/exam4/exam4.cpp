    ...
    case WM_PAINT:
        {
...

			{
				...
				TCHAR szBuf[256];
				swprintf_s(szBuf, 256, L"%c, %c, %c, %c \n\n", ch, 'A' + 1, ch + 2, '0' + 9);
				TextOut(hdc, 200, 0, szBuf, wcslen(szBuf));

				swprintf_s(szBuf, 256, L"char size :%d \n\n", sizeof(char));
				TextOut(hdc, 0, 0, szBuf, wcslen(szBuf));

				swprintf_s(szBuf, 256, L"short size :%d \n\n", sizeof(short));
				TextOut(hdc, 0, 24, szBuf, wcslen(szBuf));

				swprintf_s(szBuf, 256, L"int size :%d \n\n", sizeof(int));
				TextOut(hdc, 0, 48, szBuf, wcslen(szBuf));
				
				swprintf_s(szBuf, 256, L"long size :%d \n\n", sizeof(long));
				TextOut(hdc, 0, 72, szBuf, wcslen(szBuf));
			}
