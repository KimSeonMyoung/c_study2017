...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		TCHAR szBuf[256];
		swprintf(szBuf, 256, L"correct num : %d \n", g_number);

		CreateWindow(L"button", L"숫자맞추기", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
			0, 0, 100, 25, hWnd, (HMENU)4001, hInst, NULL);
		CreateWindow(L"edit", NULL, WS_CHILD | WS_VISIBLE | WS_BORDER,
			0, 30, 100, 25, hWnd, (HMENU)3001, hInst, NULL);
	}
	break;
    case WM_COMMAND:
        {
			...
			case 4001:
			{
				...
				nInputNum = _wtoi(szBuf);

				if (nInputNum == g_number) {
					MessageBox(hWnd, L"정답입니다!!", L"", MB_OK);
				}
				else {
					//MessageBox(hWnd, L"오답입니다!!", L"", MB_OK);
					if (nInputNum > g_number) {
						MessageBox(hWnd, L"더작은수 입니다.", L"", MB_OK);
					}
					else {
						MessageBox(hWnd, L"더큰 수 입니다.", L"", MB_OK);
					}
				}
			}
			break;
