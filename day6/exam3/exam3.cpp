    ...
    case WM_PAINT:
        {
			...
			while (nIndex < 17) {
				...
				TCHAR szBuf[256];
				swprintf_s(szBuf, 256, L"%d, %c,%d, %c,%d, %c,%d, %c,%d, %c,%d, %c",
					nIndex, nIndex,
					nIndex + (16 * 2), nIndex + (16 * 2),
					nIndex + (16 * 3), nIndex + (16 * 3),
					nIndex + (16 * 4), nIndex + (16 * 4),
					nIndex + (16 * 5), nIndex + (16 * 5)
				);
					
				TextOut(hdc, 1, 22*nIndex , szBuf, wcslen(szBuf));
