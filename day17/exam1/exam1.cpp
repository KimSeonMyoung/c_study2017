    ...
    case WM_PAINT:
        {
			...
			irr::core::vector2df vStart(-50, 100);
			irr::core::vector2df vEnd(50, -50);

			grp.DrawLine(&pen, vStart.X,vStart.Y, vEnd.X,vEnd.Y);
			plusEngine::printf(&grp, vEnd.X, vEnd.Y, L"앞");

			irr::core::line2df line1(vStart.X, vStart.Y, vEnd.X, vEnd.Y);
			irr::f32 result = line1.getPointOrientation(g_vTestPt);

			grp.DrawLine(&pen, g_vTestPt.X - 4, g_vTestPt.Y - 4, 8.0, 8.0);
			if (result > 0) {
				plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y-14, L"오른쪽 %.1lf", result);
			}
			else {
				plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y-14, L"왼쪽 %.1lf", result);
			}
			//plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y, L"&lf",result);
