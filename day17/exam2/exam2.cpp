    ...
    case WM_PAINT:
        {
			...
			grp.SetTransform(&Matrix(1, 0, 0, 1, 160, 120));
			{
				...
				grp.TranslateTransform(-4, -4);
				grp.DrawEllipse(&pen, g_vTestPt.X, g_vTestPt.Y, 8.0, 8.0);
				grp.SetTransform(&tempMat);
			}

			irr::core::vector2df polygon[] = {
				irr::core::vector2df(-50,50),
				irr::core::vector2df(-20,25),
				irr::core::vector2df(100,80),
				irr::core::vector2df(-50,50)
			};
			...
			for (int i = 0; i < 3; i++) {
				irr::core::vector2df start = polygon[i];
				irr::core::vector2df end = polygon[i+1];
				grp.DrawLine(&pen,start.X, start.Y,end.X,end.Y);

				irr::core::line2df testLine(start, end);
				
				if (testLine.getPointOrientation(g_vTestPt) < 0) {
					isInner = false;
				}
			}
			
			if (isInner) {
				plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y - 14, L"안이에요");
			}
			else {
				plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y - 14, L"밖이에요");
			}
