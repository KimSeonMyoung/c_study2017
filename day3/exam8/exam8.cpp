...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		hCheckBox1 = CreateWindow(L"button", L"선택하세요.",
			WS_CHILD | WS_VISIBLE | BS_CHECKBOX,
			20, 20, 150, 25, hWnd, (HMENU)3001, hInst, NULL);
	}
	break;
    ...
    case WM_COMMAND:
        {
			...
			case 3001:
			{
				if (SendMessage(hCheckBox1, BM_GETCHECK, 0, 0) == BST_UNCHECKED) {
					SendMessage(hCheckBox1, BM_GETCHECK, BST_CHECKED, 0);
				}
				else {
					SendMessage(hCheckBox1, BM_GETCHECK, BST_UNCHECKED, 0);
				}
			}
			break;
            case IDM_ABOUT:
